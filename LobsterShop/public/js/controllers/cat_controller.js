		...
		function($scope, $http) {
			...
			$http.get('/cart/get').success(function (data, status, headers, config) {
		        ...
		        for(var i = 0; i < $scope.cart.items.length; i++){
		        	$http
					.get('/products/' +$scope.cart.items[i].pid + '?storeId='+$scope.storeId)
					.success(
				      ...
				      $http({
			            ...
			            .success(
			            		function (data, status, headers,
			            			...
			            			if (data.flag) {
				/*
										if(getCartProductAmount(cartObj) < 100){
											  if (confirm("满100元送秘制濑尿虾1份哦,您还未满100元,确定下单吗?")) {
											window.location.href = window.location.protocol + '//' + window.location.host + '/w/pay?productAmount=' + getCartProductAmount($scope.cart) + '&storeId='+  GetQueryString('storeId')
										}else{
                                             return
                                                  }
										}*/
