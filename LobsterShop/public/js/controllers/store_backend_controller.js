...
var app = angular.module('StoreBackendApp', ['ngGrid', 'angularFileUpload', 'fundoo.services']);
...
app.controller('StoreBackendController', ['$scope', '$http', '$upload', 'createDialog', '$log', function ($scope, $http, $upload, createDialogService, $log) {
    ...
    function refreshDate(){
        $http.get('/stores?page=' + $scope.page).success(function (data, status, headers, config) {
        	$log.log(data)
            if (data.flag) {
    ...
    $scope.gridOptions = { data: 'list',
        ...
        columnDefs: [
            ...
            {field: 'area', displayName: '店铺区域', enableCellEdit: true, editableCellTemplate: cellEditableTemplate},
            {field: 'phone', displayName: '联系电话', enableCellEdit: true, editableCellTemplate: cellEditableTemplate},
            {field: 'mailbox', displayName: '邮箱',enableCellEdit: true, editableCellTemplate: cellEditableTemplate},
            {field: 'description1', displayName: '备注1', enableCellEdit: true, editableCellTemplate: cellEditableTemplate},
            {field: 'description2', displayName: '备注2', enableCellEdit: true, editableCellTemplate: cellEditableTemplate},
        ] };
        ...
        if(items.length == 0){
            ...
            if(content.id){
                bootbox.confirm("您确定要删除这个对象吗?", function(result) {
                    if(result) {
                        $http.delete('/stores/' + content.id).success(function(data, status, headers, config) {
                            if (data.flag) {
    ...
    $scope.addContent = function(){
        $scope.currentObj = {};
        createDialogService("/assets/js/controllers/store_editortemplate.html",{
            id: 'editor',
            title: '新增店铺',
            scope: $scope,
